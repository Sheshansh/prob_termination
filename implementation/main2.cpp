...
void find_states(int& begin, int &end){
	if(part(program,begin,begin+6)!="states"){
		cerr<<"No states specified"<<endl;
		exit(0);
	}
	int semicolon = -1;
	...
	if(semicolon==-1){
		cerr<<"No semicolon in fst";
		exit(0);
	}
	istringstream ss(part(program,begin+6,semicolon));
	string token;
	while(getline(ss,token,',')){
		int id = ++last_used_label;
		label_map[id] = new CFG_location("det",id);
		skip_spaces(token);
		stateId[token] = id;
	}
	begin = semicolon+1;
...
void find_transitions(string transition){
	skip_spaces(transition);
};
...
int main(){
	...
	int var=-1, states=-1, semicolon1=-1, semicolon2=-1;
	
	i=0;

	for(;i<endprog;i++){
		if(part(program,i,i+3)=="var"){
			begin = i;
			break;
		}
	}
	...
	find_states(begin,endprog);
	for(i=begin;i<endprog-10;i++){
		if(part(program,i,i+10)=="transtion"){
			int open=-1, close = -1;
			for(;i<endprog;i++){
				if(program[i]=='{'){
					open = i;
			...
			find_transitions(part(program,open+1,close));
			i = close+2;
		}
	}
	for(;i<endprog;i++){
	...
	nVariables = find_variables(begin,endprog);
