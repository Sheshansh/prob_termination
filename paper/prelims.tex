...
mathematically 
sound way. The definition of a measurable scheduler that we use is the standard 
one used when dealing with systems that exhibit both probabilistic 
and non-deterministic behaviour over a continuous state 
space~\cite{NSK:CTMDP-delayed,NK:CTMDP-bisimulation}. The definition is 
somewhat 
technical, and not substantial to 
our results, so we defer it to~\AppendixMaterial. In the rest of this work, we 
refer to measurable schedulers simply as ``schedulers.'' In particular, if the 
set of all reachable variable valuations is discrete (such as the integers), 
...
%inevitably emerge in works dealing with systems that exhibit both 
%probabilistic 
%and non-deterministic behaviour over a continuous state 
%space~\cite{NSK:CTMDP-delayed,NK:CTMDP-bisimulation}. In particular, if all 
%the 
...
%the form $F\times \Conf^{\infty}$, where $F\in \mathcal{H}$ is a measurable 
%set 
%of finite paths. 
The 
probability 
