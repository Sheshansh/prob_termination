...
pairwise
disjoint subsets  $\locs_N$, $\locs_P$, $\locs_D$, and $\locs_A$ of 
non-deterministic, 
probabilistic, deterministic, and assignment locations;
...
We call a configuration $(\loc,\vec{x})$ non-deterministic if $\loc$ is a 
non-deterministic location or if $\loc$ is an assignment location with a 
non-deterministic assignment (i.e., the only transition $\tau$ outgoing from 
$\loc$ has $\updates(\tau)=(j,\up)$ where $\up$ is a set).
...
configuration 
we have that $\sigma(\genpath)$ is a probability distribution on $\Conf$, hence 
for each measurable set of configurations $\confset$ we denote by 
...
variables in a program range over a discrete subset (such as the integers), 
...
in~\cite{CNZ16:prob-invariants-arxiv}. We point out that the 
