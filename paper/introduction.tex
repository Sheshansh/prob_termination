...
property.
The qualitative (aka almost-sure) termination problem asks given a probabilistic program
whether the program terminates with probability~1.
...
While deep theoretical results have been establishes about RSMs, 
their application to probabilistic programs with nondeterminism has been limited
to academic examples. 
For non-probabilistic programs lexicographic ranking functions provide a compositional
and practical approach for termination analysis, which has been missing for
probabilistic programs. 
In this work we introduce lexicographic RSMs and show that they present a sound
method for almost-sure termination .
We show that lexicographic RSMs enjoy the desired property of compositionality,
and for programs with linear arithmetic they can be synthesized efficiently (in 
polynomial time).
We also show with additional restrictions even asymptotic bounds on expected
termination time can be obtained through lexicographi RSMs.
